...
class PunchFinishInputService
{
    ...
    {
        // 外出時間調整が10:30よりも早く、戻り時間調整が10:45より遅ければ午前の休憩は取れないので、休憩時間から15分マイナスする
        if($out_time_adj <= "10:30:00" && $return_time_adj >= "10:45:00"){
            $rest_time -= 15;
        }
        // 外出時間調整が15:00よりも早く、戻り時間調整が15:15より遅ければ午後の休憩は取れないので、休憩時間から15分マイナスする
        if($out_time_adj <= "15:00:00" && $return_time_adj >= "15:15:00"){
            $rest_time -= 15;
        }
        // 外出時間調整が12:00よりも早く、戻り時間調整が12:15より遅ければお昼休憩は取れないので、休憩時間から60分マイナスする
