...
class DataDownloadService
{
    ...
    public function getWorkingTimeByEmployee($base_name, $base_id, $start_day, $end_day, $aggregate_unit)
    {
        ...
        // 日単位か月単位かで日付をフォーマットするか可変
        if($aggregate_unit == '日単位'){
            $employees->select(DB::raw("work_day as date, employees.employee_category_id, employee_category_name, employee_no, employee_last_name, employee_first_name, sum(working_time) as total_working_time, sum(over_time) as total_over_time, sum(rest_time + add_rest_time) as total_rest_time, sum(no_rest_time) as total_no_rest_time, bases.base_name"));
        }else{
            $employees->select(DB::raw("DATE_FORMAT(work_day, '%Y-%m') as date, employees.employee_category_id, employee_category_name, employee_no, employee_last_name, employee_first_name, sum(working_time) as total_working_time, sum(over_time) as total_over_time, sum(rest_time + add_rest_time) as total_rest_time, sum(no_rest_time) as total_no_rest_time, bases.base_name"));
        }
        ...
        foreach($employees as $employee){
            $param = [
                ...
                '休憩未取得時間' => $employee->total_no_rest_time / 60,
            ];
            $download_data[] = $param;
        }
        return $download_data;
