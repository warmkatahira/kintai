        ...
        <div class="punch_finish grid grid-cols-12 p-5">
            <div class="col-span-12 grid grid-cols-12 gap-x-4 bg-gray-200 border-8 border-dotted border-blue-500 px-5">
                @if(session('punch_type') == '退勤')
                    ...
                    <div class="text-4xl text-center col-start-1 col-span-3 row-span-1 text-white bg-blue-600 rounded-lg py-10">
                        <p class="">{{ '月間稼働設定' }}</p>
                        <p class="mt-10">{{ number_format(session('monthly_workable_time'), 2) }}</p>
                    </div>
                    <div class="text-4xl text-center col-start-4 col-span-3 row-span-1 text-white bg-blue-600 rounded-lg py-10">
                        <p class="">{{ '総稼働' }}</p>
                        <p class="mt-10">{{ number_format(session('total_month_working_time'), 2) }}</p>
                    </div>
                    <div class="text-4xl text-center col-start-7 col-span-3 row-span-1 text-white bg-blue-600 rounded-lg py-10">
                        <p class="">{{ '稼働可能残' }}</p>
                        <p class="mt-10">{{ number_format(session('workable_times'), 2) }}</p>
                    </div>
                    <div class="text-4xl text-center col-start-10 col-span-3 row-span-1 text-white bg-blue-600 rounded-lg py-10">
                        <p class="">{{ '総残業時間' }}</p>
                        <p class="mt-10">{{ number_format(session('total_over_time'), 2) }}</p>
                    </div>
                @else
                ...
                @endif
                <p class="col-start-1 col-span-12 text-center text-7xl tracking-in-expand mt-7 {{ session('message_text_color') }}">{{ session('message') }}</p>
            </div>
        </div>
    @endif
</div>